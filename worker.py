...
class CondaEnvWorker(BaseWorker):
    ...
    def _is_submission_finished(self):
        ...
        """
        return False if self._proc.returncode is None else True
    ...
    async def launch_submission(self):
        ...
        cmd_ramp = (f'{cmd_ramp} '
                    ...
                    f'--ramp_kit_dir {self.config["ramp_kit_dir"]} '
                    f'--ramp_data_dir {self.config["ramp_data_dir"]} '
                    f'--save-y-preds')
        self._proc = await asyncio.subprocess.create_subprocess_shell(
            cmd_ramp,
    ...
    async def collect_results(self):
        ...
        if self.status == 'finished' or self.status == 'running':
            ...
            self._proc_log, _ = await self._proc.communicate()
